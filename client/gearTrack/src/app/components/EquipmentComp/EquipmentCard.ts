...
@Component({
    ...
    standalone:true,
    imports:[editDialog,checkoutDialog],
 ...
 <div class="group relative border border-gray-200 rounded-xl p-4 bg-white hover:shadow-2xl transition-all duration-300 ease-in-out">
  ...
  <div class="mt-4 flex flex-wrap justify-center gap-10">
   @if(user?.role=="admin"){
    <button class="bg-[#0a0a0a] hover:bg-[#0a0a0abd]" (click)="onopenEditFlagdialog()">
       <span>
...
<button class="bg-[#0a0a0a] hover:bg-[#0a0a0abd]" (click)="ondelete(equipment._id)">
       <span>
     ...
     @if(user?.role=="staff"){
     <button (click)="onOpencheckoutdialog()" class="bg-[#0a0a0a] hover:bg-[#0a0a0abd]" (click)="ondelete(equipment._id)">
       <span>
  ...
  <checkout-dialog [checkoutFlag]="checkoutFlag" [itemId]="equipment._id" [onOpencheckoutdialog]="onOpencheckoutdialog" [onclosecheckoutDialog]="onclosecheckoutDialog"/>
</div>
