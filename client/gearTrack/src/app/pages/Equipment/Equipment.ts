    ...
    Add
        ...
        <div class="bg-white">
          <div class="mx-auto max-w-2xl   lg:max-w-7xl ">
            <h2 class="text-2xl font-bold tracking-tight ">Equipments</h2>
           @if( !EquipmentSevice.Equipment() || EquipmentSevice.Equipment().length==0 ){
             <p class="h-96 flex justify-center items-center" >{{error}}</p>
           }@else {
                <div
...
export class Equipment implements OnInit {
 ...
 openAddDialog = () => { this.addFlag.set(true); }
  ...
  getAvaibleEquipment(){
    this.loader=true;
    this.EquipmentSevice.getAllavaibleEquipment().subscribe({
      next:(res)=>{
         this.EquipmentSevice.Equipment.set(res.Equipments);
          this.loader=false;
         this.error="";
