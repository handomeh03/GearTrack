...
checkin(token:string,itemId:string):Observable<any>{
   return this.http.patch(`http://localhost:8080/api/Equipment/${itemId}/checkin`,{},{
    headers:{
       Authorization: `Bearer ${token}`
    }
   })
}
checkout(token:string,itemId:string,startDate:string,endDate:string):Observable<any>{
  const body={startDate,endDate};
  return this.http.post(`http://localhost:8080/api/Equipment/${itemId}/checkout`,body,{
    headers:{
       Authorization: `Bearer ${token}`
    }
  });
